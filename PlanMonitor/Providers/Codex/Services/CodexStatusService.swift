...
struct CodexStatusSnapshot: Sendable {
    let status: CodexSystemStatus
    let sourceUpdatedAt: Date?
    let fetchedAt: Date
}

enum CodexSystemStatus: Sendable {
    case operational
    case degraded
    case outage
...
/// status.openai.com is a Statuspage-compatible site, so this mirrors `ClaudeStatusService`
/// against the same `/api/v2/status.json` shape.
final class CodexStatusService {
    ...
    func fetchStatus() async throws -> CodexStatusSnapshot {
        ...
        return CodexStatusSnapshot(
            status: CodexSystemStatus(indicator: payload.status.indicator),
